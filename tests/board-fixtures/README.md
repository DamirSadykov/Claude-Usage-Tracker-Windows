...
|---|---|---|---|
| `v1/empty.json` | пустая доска до миграции 1→2 | читается-но-не-пишется: миграцию 1→2 выполняет только приложение, запись отказывает кодом 4 без бэкапа | читается, мигрирует в v2 |
| `v1/full.json` | три узла до миграции: 1) статус `pending`, `plan` пуст; 2) `plan` — указатель на `.claude/phases/`, архивируется в комментарий; 3) `plan` — цельный ритуальный markdown с замыслом и шагами, делится на `description`+`plan` | читается-но-не-пишется: миграцию 1→2 выполняет только приложение, запись отказывает кодом 4 без бэкапа | читается, мигрирует в v2 |
...
|---|---|---|
| `v1/empty.json` (version 1) | читается-но-не-пишется: миграцию 1→2 выполняет только приложение, запись отказывает кодом 4 без бэкапа. Покрыто: `describe("matrix row: v1/*.json (version 1) — CLI v2 writer")` → `it("v1/empty.json is readable but a CLI write refuses without changing it or creating a backup")` | читается, мигрирует в v2. Покрыто: `v1_empty_fixtures_round_trips_to_v2` |
| `v1/full.json` (version 1) | читается-но-не-пишется: миграцию 1→2 выполняет только приложение, запись отказывает кодом 4 без бэкапа. Покрыто: `describe("matrix row: v1/*.json (version 1) — CLI v2 writer")` → `it("v1/full.json is readable but a CLI write refuses without changing it or creating a backup")` | читается, мигрирует в v2 (статус, архив `.claude/phases/`-указателя в комментарий, разбор ритуального markdown), пишет v2. Покрыто: `v1_full_fixtures_migrates_plan_roles_and_status_on_load` |
