...
export function buildEnterContext() {
  return [
    ...
    "   done, what was decided and why (\"должно X, решили Y, потому что Z\"). On",
    "   recording it becomes the task's DESCRIPTION (the theme root's for a",
    "   multi-session plan) — and ONLY the description: the plan field will hold",
    "   the steps, never a copy of the vision.",
...
export function buildExitContext(warnings) {
  const lines = [
    ...
    "",
    "Field roles, one line each: description = WHAT & WHY (a theme root's = the",
    "VISION); plan = HOW only (steps + order, rewritten on re-plan); handoff = the",
    "result baton; comments = the journal. Never write the same text into two.",
    "",
    ...
    '  4. Steps:     <cli> todos set-plan <root> --text "<STEPS + ORDER sections ONLY>"',
    "                — the vision is already in the description; do NOT repeat it here.",
    "",
    "Plan of ONE step: no theme — record onto the task you are working on (or a",
    "new one):",
    '  - vision   →  <cli> todos set-description <id> --text "<the VISION paragraph>"',
    "                (only when its description is still empty — never overwrite one)",
    '  - steps    →  <cli> todos set-plan <id> --text "<STEPS + ORDER sections ONLY>"',
    "",
