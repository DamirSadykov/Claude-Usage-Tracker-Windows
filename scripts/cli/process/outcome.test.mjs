...
describe("parseTouchedFiles", () => {
  it("takes writes as touches and never counts a Read as producing", () => {
    const raw = [
      ...
      toolLine("Write", "D:\\p\\scripts\\cli\\process\\outcome.mjs", "2026-07-28T10:01:00.000Z"),
      toolLine("Edit", "D:/p/scripts/cli/board/todos.mjs", "2026-07-28T10:02:00.000Z"),
    ].join("\n");
    const { touches, last_ts } = parseTouchedFiles(raw);
    expect(touches.map((t) => [t.path, t.mutates])).toEqual([
      ["D:/p/src/a.rs", false],
      ["D:/p/scripts/cli/process/outcome.mjs", true],
      ["D:/p/scripts/cli/board/todos.mjs", true],
    ]);
...
describe("path matching", () => {
  it("matches a repo-relative declaration against an absolute Windows path", () => {
    expect(pathMatches("scripts/cli/process/outcome.mjs", "D:/p/scripts/cli/process/outcome.mjs")).toBe(true);
    expect(pathMatches("scripts/cli/process/outcome.mjs", "D:/p/scripts/cli/process/outcome.test.mjs")).toBe(false);
  });

  it("tells a path-shaped declaration from a prose one", () => {
    expect(isPathLike("scripts/cli/process/outcome.mjs")).toBe(true);
...
describe("buildOutcomeReport", () => {
  ...
  const data = () =>
    board(
      [1, { produces: ["scripts/cli/process/outcome.mjs"] }],
      [2, { depends_on: ["id-1"] }],
    );
  ...
  it("promised output produced and taken by a dependent -> ok, consumed", () => {
    ...
    const report = buildOutcomeReport({
      ...
      touches: touchesOf(
        ["s1", [wrote("D:/p/scripts/cli/process/outcome.mjs", "T1")]],
        ["s2", [read("D:/p/scripts/cli/process/outcome.mjs", "T3")]],
      ),
    });
  ...
  it("promised output produced but taken by nobody -> still ok, reported unclaimed", () => {
    ...
    expect(report.unconsumed).toEqual(["scripts/cli/process/outcome.mjs"]);
    expect(report.missing).toEqual([]);
    expect(report.outcome).toBe("ok");
  });
  ...
  it("promised output never produced -> issue with missing:<path>", () => {
    ...
    const report = buildOutcomeReport({
      ...
      touches: touchesOf(["s1", [wrote("D:/p/scripts/cli/board/todos.mjs", "T1")]]),
    });
    expect(report.missing).toEqual(["scripts/cli/process/outcome.mjs"]);
    expect(report.side_effects).toEqual(["D:/p/scripts/cli/board/todos.mjs"]);
    expect(report.outcome).toBe("issue");
    expect(report.outcome_reason).toBe("missing:scripts/cli/process/outcome.mjs");
  });
  ...
  it("a write outside the block window is not this step's output", () => {
    ...
    const report = buildOutcomeReport({
      ...
      touches: touchesOf(["s1", [wrote("D:/p/scripts/cli/process/outcome.mjs", "T1")]]),
    });
    expect(report.outcome_reason).toBe("missing:scripts/cli/process/outcome.mjs");
  });

  it("verify declared but not run -> outcome NOT finalized", () => {
    const d = board([1, { produces: ["scripts/cli/process/outcome.mjs"], verify: "npx vitest run" }]);
    const report = buildOutcomeReport({
      data: d,
      todo: d.todos[0],
      blocks: [at("id-1", "s1", "T1", "T2")],
...
describe("outcome --write (end to end)", () => {
  ...
  beforeEach(() => seed([todo(1, { produces: ["scripts/cli/process/outcome.mjs"] })]));
  ...
  it("writes exactly outcome, outcome_reason and outcome_at and moves nothing else", () => {
    ...
    expect(after.outcome_reason).toBe("missing:scripts/cli/process/outcome.mjs");
  ...
  it("without --write it only prints; --json carries the same reconciliation", () => {
    ...
    expect(json.missing).toEqual(["scripts/cli/process/outcome.mjs"]);
  ...
  it("refuses --write on a future-version board with exit 4 and no backup", () => {
    seed([todo(1, { produces: ["scripts/cli/process/outcome.mjs"] })]);
