...
import { formatSpecSections, recordSpecBaseline } from "../spec/board-ritual.mjs";
import { resolveAddress } from "../spec/spec.mjs";
...
describe("formatDeclarations", () => {
  it("prints the declarations of a node on one compact line", () => {
    const out = formatDeclarations(
      {
        ...
        status: "queue",
        kind: "auto",
        produces: ["scripts/cli/board/todos.mjs", "scripts/cli/board/todos.test.mjs"],
        verify: "npm test",
        retry_limit: 2,
        budget_usd: 2,
        on_issue: "impl-id",
    ...
    expect(out).toContain("produces: scripts/cli/board/todos.mjs, scripts/cli/board/todos.test.mjs");
...
describe("declaration commands", () => {
  ...
  it("produces add declares outputs and produces list reads them back", () => {
    ...
    expect(read(1).produces).toEqual([
      "scripts/cli/board/todos.mjs",
      "scripts/cli/board/todos.test.mjs",
    ]);
    const listed = run("produces", "list", "t#1");
    expect(listed).toContain("scripts/cli/board/todos.mjs");
