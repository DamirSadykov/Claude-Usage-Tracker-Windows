...
function main() {
  ...
  if (active.length) {
    ...
    const context = [
      ...
      `  carry handoff   → moving a task to in_progress AUTO-prints the handoff it inherits from its deps; \`<cli> todos handoff <task>\` re-reads it  ·  \`<cli> todos handoff set <task> --text "<body>"\`  leaves yours for dependents`,
