...
fn reaffirm_at(lock_path: &Path) -> Result<(), LockError> {
    ...
    match read_lock(lock_path) {
        ReadOutcome::Content(content) => match parse_holder(&content) {
            ...
            None => Err(LockError::Unreadable {
                path: lock_path.to_path_buf(),
            }),
        },
        ReadOutcome::Missing | ReadOutcome::Unreadable => Err(LockError::Unreadable {
            path: lock_path.to_path_buf(),
        }),
    }
}
...
mod tests {
    ...
    fn unreadable_lock_younger_than_30s_is_busy_then_unreadable_after_timeout() {
        ...
        match err {
            LockError::Unreadable { path } => {
                ...
                assert_eq!(
                    err_display(&LockError::Unreadable {
                        path: lock_path.clone()
                    }),
                    format!(
                        "board locked (contents unreadable): {}",
                        lock_path.display()
                    )
                );
            }
    ...
    fn reentrant_acquire_does_not_wait_or_recreate() {
        ...
        let created = std::fs::metadata(lock_path_for(board.path()))
            .unwrap()
            .modified()
            .unwrap();
        ...
        let still = std::fs::metadata(lock_path_for(board.path()))
            .unwrap()
            .modified()
            .unwrap();
    ...
    fn is_stale_pure_function() {
        ...
        let bad_at_content =
            serde_json::json!({ "pid": 1234, "writer": "app", "at": "not-a-date" }).to_string();
        assert!(!is_stale(&bad_at_content, Duration::from_secs(999), |_| {
            true
        }));
        assert!(is_stale(&bad_at_content, Duration::from_secs(0), |_| false));

        let bad_pid_content =
            serde_json::json!({ "pid": -5, "writer": "app", "at": now_ms() }).to_string();
        assert!(!is_stale(&bad_pid_content, Duration::from_secs(10), |_| {
            true
        }));
        assert!(is_stale(&bad_pid_content, Duration::from_secs(31), |_| {
            true
        }));
    ...
    fn lock_file_path_and_content_match_node_shape() {
        ...
        let re_millis_z = |s: &str| s.len() == 24 && s.ends_with('Z') && s.as_bytes()[19] == b'.';
    ...
    fn live_pid_lock_is_never_stale_by_age_alone() {
        ...
        assert!(
            content.contains("\"writer\":\"cli\""),
            "a live holder's lock must not be swept"
        );
    }
    ...
    fn reaffirm_leaves_the_lock_file_untouched_while_still_ours() {
        ...
        assert_eq!(
            before, after,
            "reaffirm only re-reads ownership — it must not rewrite the file"
        );
        drop(lock);
    }
