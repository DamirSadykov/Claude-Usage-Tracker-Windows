...
pub fn build(
    ...
    let change_roots: Vec<&str> = nodes
        ...
        .collect();
    let node_order: HashMap<&str, usize> = ids
        .iter()
        .enumerate()
    ...
    for id in &ids {
        let Some(t) = by_id.get(id.as_str()) else {
            continue;
        };
...
mod tests {
    use super::*;
    use crate::board::todos::{StatusChange, CURRENT_VERSION};
    ...
    fn an_unworked_change_root_is_unknown_too() {
        let mut board = sample();
        board.todos[0].status_history = history(&[
            ("backlog", "2026-07-20T09:00:00Z"),
            ("done", "2026-07-22T03:20:00Z"),
        ]);
        let g = built(&board);
        let root = node(&g, 273);
    ...
    fn an_outcome_predicate_is_marked_and_ok_reads_differently_from_issue() {
        ...
        assert!(
            !text.contains("class t274 issue"),
            "a passed node is not painted"
        );
    ...
    fn labels_survive_quotes_arrows_brackets_and_pipes() {
        ...
        let child = todo(
            "c",
            307,
            "Кавычки «ёлочки» и \"канон\", скобки (a;b) [x] {y}",
        );
    ...
    fn an_empty_block_is_unknown_while_a_zero_priced_one_is_an_honest_zero() {
        ...
        assert_eq!(
            a.blocks, 1,
            "the degenerate block stays visible as evidence"
        );
...
mod change_record_tests {
    use super::*;
    use crate::board::todos::{Change, StatusChange, CURRENT_VERSION};
    ...
    fn board() -> TodoFile {
        TodoFile {
            ...
            todos: vec![
                worked(
                    todo("t-1", 1, Some("ch-1")),
                    "2026-08-01T10:00:00Z",
                    "2026-08-01T12:00:00Z",
                ),
                worked(
                    todo("t-2", 2, Some("ch-1")),
    ...
    fn a_change_resolves_by_address_number_and_id() {
        ...
        for r in ["c#3", "c3", "3", "ch-1"] {
            assert_eq!(
                b.find_change(r).map(|c| c.id.as_str()),
                Some("ch-1"),
                "ref {r}"
            );
        }
