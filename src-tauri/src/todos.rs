    ...
    match std::fs::OpenOptions::new().write(true).create_new(true).open(&backup_path) {
        Ok(mut f) => {
            ...
            if let Err(e) = f.write_all(raw.as_bytes()) {
                warn!("board recovery: backup write failed for {}: {e}", path.display());
                return None;
            }
            Some(backup_path)
        }
...
fn file_or_refusal(path: &Path, outcome: LoadOutcome) -> Result<TodoFile, String> {
    match outcome {
        ...
        LoadOutcome::Unreadable { reason, backup } => Err(format!(
            ...
            path.display(),
            backup.as_ref().map(|p| p.display().to_string()).unwrap_or_else(|| "none".to_string())
        )),
...
mod tests {
    ...
    fn load_for_write_refuses_unreadable_and_future_version_with_spec_messages() {
        let (unreadable_dir, unreadable_path) = staged_fixture("write-unreadable", "corrupt/not-object.json");
        let err = load_for_write(&unreadable_path).unwrap_err();
        assert!(err.starts_with("board unreadable (not-object): "), "got: {err}");
