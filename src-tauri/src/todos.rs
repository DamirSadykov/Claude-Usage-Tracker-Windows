...
/// `status` is one of `pending` | `in_progress` | `done`. Optional fields are
/// omitted from the file when empty to keep hand-edits readable.
...
/// view, and the hook must never fail a session over a bad file.
...
mod tests {
    ...
    fn upsert_adds_then_updates_in_place() {
        ...
        upsert(&mut f, todo("a", "pending"), "T1");
    ...
    fn delete_removes_by_id() {
        let mut f = TodoFile::default();
        upsert(&mut f, todo("a", "pending"), "T1");
        upsert(&mut f, todo("b", "pending"), "T1");
        delete(&mut f, "a");
        assert_eq!(f.todos.len(), 1);
