...
use reqwest::header::{
    HeaderMap, HeaderValue, ACCEPT, CONTENT_TYPE, REFERER, SERVER, SET_COOKIE, USER_AGENT,
};
    ...
    if !session_key_is_cookie_safe(session_key) {
        ...
        return Err(FetchError::new(
            ...
            message_for(
                Verdict::BadSessionKey,
                StatusCode::OK,
                "недопустимые символы (пробел, `;`, не-ASCII)",
            ),
        ));
    }
    ...
    if !names.iter().any(|n| n == "sessionKey") {
        ...
        return Err(FetchError::new(
            ...
            message_for(
                Verdict::BadSessionKey,
                StatusCode::OK,
                "хранилище cookie его не приняло",
            ),
        ));
    }
    Ok(())
...
impl Verdict {
    ...
    pub fn blame(self) -> &'static str {
        match self {
            ...
            Verdict::SessionRejected | Verdict::LoginRedirect | Verdict::BadSessionKey => {
                "session_key"
            }
...
impl FetchError {
    fn new(verdict: Verdict, message: String) -> Self {
        Self {
            message,
            session_expired: verdict.session_expired(),
            verdict,
        }
    }
}
...
fn message_for(verdict: Verdict, status: StatusCode, detail: &str) -> String {
    match verdict {
        ...
        Verdict::SessionRejected => format!(
            "API вернул {} — ключ сессии недействителен или истёк",
            status
        ),
        Verdict::LoginRedirect => {
            "Ключ сессии истёк — Claude вернул страницу входа вместо данных".to_string()
        }
        Verdict::OrgNotFound => format!("API вернул {} — проверьте Organization ID", status),
        Verdict::ClaudeOutage => format!(
            "API вернул {} — сервис Claude временно недоступен, повторите позже",
            status
        ),
...
pub async fn fetch_usage(session_key: &str, org_id: &str) -> Result<UsageData, FetchError> {
    ...
    let usage_resp = match usage_resp {
        ...
        Err(e) => {
            ...
            return Err(FetchError::new(
                Verdict::Network,
                message_for(Verdict::Network, StatusCode::OK, &detail),
            ));
        }
    };
    ...
    if !facts.status.is_success() {
        ...
        return Err(FetchError::new(
            verdict,
            message_for(verdict, facts.status, ""),
        ));
    }
    ...
    let api: ApiResponse = match serde_json::from_str(&body) {
        ...
        Err(e) => {
            let verdict =
                classify_response(facts.status, facts.cf_mitigated, &facts.final_url, &body);
            log_verdict(verdict, &facts, &body);
            return Err(FetchError::new(
                verdict,
                message_for(verdict, facts.status, &e.to_string()),
            ));
        }
    };
    ...
    let prepaid: Option<ApiPrepaidCredits> = match credits_resp {
        ...
        Err(e) => {
            debug!(
                "fetch_usage: credits request failed, ignored ({})",
...
pub async fn ensure_project(
    ...
    let list_url = format!("https://claude.ai/api/organizations/{}/projects", org_id);
    let resp = client
        .get(&list_url)
    ...
    let create_url = format!("https://claude.ai/api/organizations/{}/projects", org_id);
...
pub async fn start_session_unchecked(
    ...
    let _ = client.delete(&del_url).headers(headers).send().await;
...
mod tests {
    ...
    fn scoped_weekly_limits_are_extracted_by_model() {
        ...
        assert_eq!(
            u.scoped_weekly.len(),
            1,
            "only the weekly_scoped row is kept"
        );
    ...
    fn cloudflare_challenge_is_told_apart_from_a_rejected_key() {
        ...
        assert!(
            !is_cloudflare_challenge(rejected),
            "a real auth failure must stay an auth failure"
        );

        // Real usage payloads must never trip the check either.
        assert!(!is_cloudflare_challenge(
            r#"{"five_hour":{"utilization":19.0}}"#
        ));
    }
    ...
    fn challenge_error_does_not_claim_the_key_expired() {
        ...
        assert!(
            !e.session_expired,
            "the UI must not ask for a new key on a challenge"
        );
    ...
    fn verdict_names_the_guilty_side() {
        ...
        let rejected =
            r#"{"type":"error","error":{"details":{"error_code":"account_session_invalid"}}}"#;

        assert_eq!(
            classify_response(StatusCode::FORBIDDEN, false, &usage, challenge),
            Verdict::CloudflareChallenge
        );
        assert_eq!(
            classify_response(StatusCode::FORBIDDEN, true, &usage, "{}"),
            Verdict::CloudflareChallenge
        );
        assert_eq!(
            classify_response(StatusCode::FORBIDDEN, false, &usage, rejected),
            Verdict::SessionRejected
        );
        assert_eq!(
            classify_response(StatusCode::UNAUTHORIZED, false, &usage, rejected),
            Verdict::SessionRejected
        );
        assert_eq!(
            classify_response(StatusCode::NOT_FOUND, false, &usage, "{}"),
            Verdict::OrgNotFound
        );
        assert_eq!(
            classify_response(StatusCode::SERVICE_UNAVAILABLE, false, &usage, "down"),
            Verdict::ClaudeOutage
        );
        assert_eq!(
            classify_response(StatusCode::OK, false, &usage, "<!DOCTYPE html>"),
            Verdict::LoginRedirect
        );
        let login = Url::parse("https://claude.ai/login").unwrap();
        assert_eq!(
            classify_response(StatusCode::OK, false, &login, "{}"),
            Verdict::LoginRedirect
        );
        assert_eq!(
            classify_response(StatusCode::OK, false, &usage, "[]"),
            Verdict::BadJson
        );
        assert_eq!(
            classify_response(StatusCode::TOO_MANY_REQUESTS, false, &usage, "{}"),
            Verdict::HttpOther
        );
    ...
    async fn cookie_store_sends_cloudflare_cookie_back_only_without_a_manual_cookie_header() {
        ...
        assert!(
            seen[0].contains("sessionKey=sk-test") && !seen[0].contains("__cf_bm"),
            "first visit: {}",
            seen[0]
        );
        assert!(
            seen[1].contains("sessionKey=sk-test") && seen[1].contains("__cf_bm=cf-token"),
            "second visit: {}",
            seen[1]
        );
        assert!(
            !seen[2].contains("__cf_bm"),
            "manual Cookie header must silence the store (0.13.1 bug): {}",
            seen[2]
        );
    }
    ...
    fn set_cookie_names_are_logged_without_values() {
        ...
        h.append(
            SET_COOKIE,
            HeaderValue::from_static("__cf_bm=secret; Path=/; HttpOnly"),
        );
        h.append(
            SET_COOKIE,
            HeaderValue::from_static("cf_clearance=secret2; Path=/"),
        );
