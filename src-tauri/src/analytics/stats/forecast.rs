...
fn mean_hourly_rate(
    ...
    for h in 1..=lookback_hours {
        ...
        match (pct_at(series, start), pct_at(series, end)) {
            ...
            _ => {
                if cc_witness_available && !has_activity_in_range(cc_activity, start, end) {
...
fn recent_hourly_rate(series: &Series, now: DateTime<Utc>, window_min: i64) -> Option<f64> {
    if window_min <= 0 {
        return None;
    }
...
impl StatsDb {
    ...
    pub fn forecast(
        ...
        let cc_activity: Vec<DateTime<Utc>> = {
            let conn = self.conn.lock().unwrap();
            let mut stmt =
                conn.prepare("SELECT ts FROM cc_usage WHERE ts >= ?1 ORDER BY ts ASC")?;
        ...
        let (fh_med, fh_cov) = mean_hourly_rate(&five_series, &cc_activity, now, LOOKBACK_HOURS);
        let (sd_med, sd_cov) = mean_hourly_rate(&seven_series, &cc_activity, now, LOOKBACK_HOURS);
        let (ex_med, ex_cov) = mean_hourly_rate(&extra_series, &cc_activity, now, LOOKBACK_HOURS);
