...
fn cache_savings_for(
    ...
    let rows: Vec<(String, i64, i64)> = match project {
        Some(p) => stmt
            .query_map(params![from, to, p], map)?
            .collect::<Result<_, _>>()?,
        None => stmt
    ...
    for (model, cr, cc) in rows {
        if let Some((pin, _pout)) = crate::analytics::cc::price_per_mtok(&model) {
...
fn trend_metrics_for(
    ...
    let productivity = productivity_for(conn, from, to, None, totals.cost, totals.total_tokens)?;
...
fn productivity_for(
    ...
    let (git_commits, git_pushes): (i64, i64) = match project {
        Some(p) => conn.query_row(&git_sql, params![from, to, p], |r| {
            Ok((r.get(0)?, r.get(1)?))
        })?,
...
fn subagent_efficacy(
    ...
    let mut stmt = conn.prepare(&sql)?;
    let mut with_sub = EfficacyBucket {
        sessions: 0,
        main_msgs: 0,
        main_cost: 0.0,
    };
    let mut without_sub = EfficacyBucket {
        sessions: 0,
        main_msgs: 0,
        main_cost: 0.0,
    };
...
fn build_insights(
    ...
    if cache_total > 100_000 {
        ...
        if churn >= 15.0 {
            ...
            let mut top: Vec<&SessionUsage> =
                sessions.iter().filter(|s| s.cache_create > 0).collect();
    ...
    if !cold_rewrites.is_empty() {
        ...
        if total_cost >= 1.0 {
            ...
            let affected: Vec<serde_json::Value> = by_session
                ...
                .take(5)
                .map(
                    |(sid, proj, cost)| json!({ "session_id": sid, "project": proj, "cost": cost }),
                )
                .collect();
