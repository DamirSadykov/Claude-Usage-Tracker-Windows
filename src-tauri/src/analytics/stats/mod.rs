...
mod tests {
    use super::*;
    use crate::analytics::usage::UsageData;
    ...
    fn v7_recomputes_cost_for_current_prices() {
        ...
        let cost = |id: &str| -> f64 {
            conn.query_row("SELECT cost FROM cc_usage WHERE message_id=?1", [id], |r| {
                r.get(0)
            })
            .unwrap()
        };
    ...
    fn record_snapshot_roundtrip() {
        let db = mem_db();
        let data = UsageData {
            five_hour: crate::analytics::usage::UsageTier {
                percent_used: 42.0,
                reset_at: Some("2026-01-01T00:00:00Z".into()),
                is_limited: false,
            },
            seven_day: crate::analytics::usage::UsageTier {
                percent_used: 7.0,
                reset_at: None,
                is_limited: false,
            },
            ...
            extra_usage: Some(crate::analytics::usage::ExtraUsage {
                used_credits: 8.88,
                monthly_limit: 30.0,
                utilization: 29.6,
                currency: "USD".into(),
    ...
    fn cc_file_state_roundtrip() {
        ...
        db.cc_set_file_state("a.jsonl", 123, "2026-01-01T00:00:00Z")
            .unwrap();
        assert_eq!(
        ...
        // upsert overwrites
        db.cc_set_file_state("a.jsonl", 456, "2026-01-02T00:00:00Z")
            .unwrap();
    ...
    fn analytics_computes_cache_hit_ratio() {
        ...
        let a = db
            .analytics("2026-01-01T00:00:00Z", "2026-01-02T00:00:00Z")
            .unwrap();
        assert!(
            (a.totals.cache_hit_ratio - 0.9).abs() < 1e-9,
            "{}",
            a.totals.cache_hit_ratio
        );
    }
    ...
    fn analytics_computes_cache_savings_per_model() {
        ...
        // (4.5 − 1.25) + 2.7 = 5.95
        assert!(
            (a.totals.cache_savings_usd - 5.95).abs() < 1e-9,
            "{}",
            a.totals.cache_savings_usd
        );
    }
    ...
    fn cache_savings_can_be_negative_and_skips_unknown_model() {
        ...
        let a = db
            ...
            .unwrap();
        assert!(
            (a.totals.cache_savings_usd + 1.25).abs() < 1e-9,
            "{}",
            a.totals.cache_savings_usd
        );
    }
    ...
    fn analytics_aggregates_models_and_totals() {
        ...
        let a = db
            .analytics("2026-01-01T00:00:00Z", "2026-01-03T00:00:00Z")
            .unwrap();
    ...
    fn cc_upsert_backfills_null_project() {
        ...
        assert_eq!(
            db.cc_upsert(&[with_proj]).unwrap(),
            0,
            "dedup still ignores the duplicate insert"
        );
    ...
    fn cost_in_sums_only_window() {
        ...
        .unwrap();
        let cost = db
            .cost_in("2026-01-01T00:00:00Z", "2026-01-02T00:00:00Z")
            .unwrap();
        assert!(
            (cost - 3.5).abs() < 1e-9,
            "only Jan 1 rows count, got {cost}"
        );
        // empty window → zero
        assert_eq!(
            db.cost_in("2026-02-01T00:00:00Z", "2026-02-02T00:00:00Z")
                .unwrap(),
            0.0
        );
    }
    ...
    fn forecast_measures_rate_eta_and_warns() {
        ...
        let fh = f.five_hour;
        assert!(
            (fh.rate_per_hour - 20.0).abs() < 1e-6,
            "rate {}",
            fh.rate_per_hour
        );
    ...
    fn forecast_unknown_when_history_doesnt_span_an_hour() {
        ...
        let fh = db.forecast(60, now).unwrap().five_hour;
        assert!(
            fh.eta_minutes.is_none(),
            "no fully covered hour bucket → no ETA"
        );
    ...
    fn forecast_spike_amortised_into_week_average() {
        ...
        assert!(
            f.seven_day.coverage_hours >= 100,
            "{}",
            f.seven_day.coverage_hours
        );
    ...
    fn forecast_mean_projects_chronic_burn_to_warn() {
        ...
        assert!(
            (week.rate_per_hour - 0.5).abs() < 1e-6,
            "{}",
            week.rate_per_hour
        );
    ...
    fn forecast_forward_fills_when_now_is_slightly_after_latest() {
        ...
        assert!(
            fh.coverage_hours >= 1,
            "forward-fill should keep h=1 covered"
        );
    ...
    fn forecast_extra_has_eta_but_no_allowed() {
        ...
        assert!(
            ex.allowed_per_hour.is_none(),
            "extra usage has no reset → no allowed pace"
        );
    ...
    fn forecast_does_not_invent_idle_when_cc_data_is_absent() {
        ...
        assert!(
            (week.rate_per_hour - 2.0).abs() < 1e-6,
            "{}",
            week.rate_per_hour
        );
    }
    ...
    fn forecast_five_hour_recent_ignores_reset_crossing() {
        ...
        assert!(
            fh.rate_per_hour >= 0.0,
            "rate must not go negative: {}",
            fh.rate_per_hour
        );
        let mean = 20.0 / 167.0;
        assert!(
