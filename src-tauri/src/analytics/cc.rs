...
pub fn cost_for(model: &str, input: i64, output: i64, cache_create: i64, cache_read: i64) -> f64 {
...
pub fn ingest(base: &Path, db: &StatsDb) -> Result<usize, String> {
    ...
    for path in files {
        ...
        if !parsed.turns.is_empty() {
            db.cc_turn_upsert(&parsed.turns)
                .map_err(|e| e.to_string())?;
        }
        if !parsed.agents.is_empty() {
            db.cc_agent_upsert(&parsed.agents)
                .map_err(|e| e.to_string())?;
        }
...
mod tests {
    ...
    fn parse_tool_results_ignores_non_user_and_string_content() {
        ...
        assert!(parse_tool_results(
            r#"{"type":"user","timestamp":"t","message":{"content":"hello"}}"#
        )
        .is_empty());
    }
    ...
    fn parse_turn_line_rejects_zero_duration_and_non_system() {
        ...
        let wrong_subtype =
            r#"{"type":"system","subtype":"other","durationMs":5,"uuid":"u","timestamp":"t"}"#;
