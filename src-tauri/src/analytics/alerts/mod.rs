...
mod tests {
    use super::*;
    use crate::analytics::stats::UsageDelta;
    use crate::analytics::usage::{ExtraUsage, UsageData, UsageTier};
    ...
    fn forecast_fires_within_window() {
        ...
        // fires only once
        assert!(eng2
            .evaluate(&u2, &c, 0, Some(&steep), None, false)
            .is_empty());
    }
    ...
    fn warm(id: &str, ts: &str, messages: i64) -> crate::analytics::alerts::ActiveSession {
    ...
    fn insight_cause(out: &[AlertEvent]) -> Option<String> {
        match out {
            [AlertEvent::Insight { params, .. }] => params
                .get("cause")
                .and_then(|v| v.as_str())
                .map(String::from),
            _ => None,
    ...
    fn long_session_fires_once_and_rearms_on_new_session() {
        ...
        assert!(is_insight(
            &eng.evaluate(&u, &c, 0, None, None, false),
            "long_session"
        ));
    ...
    fn runtime_master_toggle_and_per_kind_gate() {
        ...
        assert!(is_insight(
            &eng2.evaluate(&u, &c, 0, None, None, false),
            "cold_rewrites"
        ));
    }
